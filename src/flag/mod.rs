...
pub fn run(request: FlagRequest) -> Result<FlagStats, CsvOpsError> {
    ...
    let transform = FlagTransform::new(compiled);

    // 変換結果は一旦 UTF-8 で Vec<u8> に書き出し、後で出力エンコーディングへ変換する
    let mut utf8_buf = Vec::new();
    let mut stats = FlagStats::new(out_cols);
    {
        ...
        for result in rdr.records() {
            ...
            // apply_record はルール毎の真偽値を末尾に追加し、bool 列を返す
            let flags = transform.apply_record(&mut record)?;
            for (i, &matched) in flags.iter().enumerate() {
                if matched {
                    stats.per_rule[i].matched_rows += 1;
                }
            }
