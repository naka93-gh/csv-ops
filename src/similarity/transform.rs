...
use super::stats::SimilarityStats;
...
impl SimilarityTransform {
    pub fn new(config: SimilarityConfig, delimiter: u8) -> Self {
        Self {
            ...
            delimiter,
            compiled: Vec::new(),
            stats: SimilarityStats::new(Vec::new()),
        }
    }
}
...
impl RecordTransform for SimilarityTransform {
    fn init(
        ...
        self.stats = SimilarityStats::new(out_cols);
    ...
    fn on_record(&mut self, record: &mut StringRecord, row: u64) -> Result<(), CsvOpsError> {
        ...
        for (i, rule) in self.compiled.iter().enumerate() {
            ...
            // 同点は辞書記述順で先勝ち。警告を stderr へ出す
            if result.tie {
                eprintln!(
                    "警告: 行 {} ({}) で同点マッチ。辞書記述順で先勝ちしました",
                    row, rule.out_col
                );
                self.stats.per_rule[i].tie_rows += 1;
            }
            ...
            if result.score >= rule.threshold {
                ...
                self.stats.per_rule[i].no_match_rows += 1;
